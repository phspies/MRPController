...
namespace MRPService.Portal.Classes.Static_Classes.Background_Classes
{
    class PerformanceWorker
    {
        ...
        private static CloudMRPPortal _cloud_movey = new CloudMRPPortal();
                ...
                if (_workloads != null)
                {
                    foreach (var _workload in _workloads.Where(x => x.enabled == true && x.iplist != null))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            MRPWorkloadCRUDType _error_update_workload = new MRPWorkloadCRUDType();

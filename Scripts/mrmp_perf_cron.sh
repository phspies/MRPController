...
# **************************************************************************
#  Module:  UNIX MRMP Performance cron Script
...
TMPOUT=$MRMPDATADIR/TmpPerf_${HOST}_${DATE}.tmp
OUTPUT=$MRMPDATADIR/Perf_${HOST}_${DATE}.txt

if [ "$OS_TYPE" = "SunOS" ] ; then
  ksh $MRMPDIR/bin/mrmp_perf.sh > $TMPOUT 2>/dev/null
else
  $MRMPDIR/bin/mrmp_perf.sh > $TMPOUT 2>/dev/null
fi

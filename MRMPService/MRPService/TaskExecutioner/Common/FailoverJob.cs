    ...
    partial class Common
    {
        static public void FailoverDoubleTakeJob(MRPTaskType _mrmp_task)
        {
            ...
            try
            {
                ...
                MRMPServiceBase._mrmp_api.task().progress(_mrmp_task.id, String.Format("Successfully migrated {0} to {1}", _source_workload.hostname, _target_workload.hostname), 99);
                MRMPServiceBase._mrmp_api.task().successcomplete(_mrmp_task.id, String.Format("Successfully migrated {0} to {1}", _source_workload.hostname, _target_workload.hostname));
            }
            catch (Exception ex)
            {

...

{
    public class DatacenterDiscovery
    {
        public static async void DatacenterDiscoveryDo(MRPTaskType payload)
        {
            ...
            {
                await MRMPServiceBase._mrmp_api.task().progress(payload, String.Format("Something went wrong, {0}", ex.Message), 30);
                await MRMPServiceBase._mrmp_api.task().failcomplete(payload, String.Format("Something went wrong, {0}", ex.Message));
            }
        }
    }
}

...
namespace MRMPService.Modules.MCP
{
    ...
    {
        public static void PreviewCG(MRPTaskType _task, MRPPlatformType _platform, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, List<MRPWorkloadPairType> _workloadpairs, MRPManagementobjectSnapshotType _snapshot, float _start_progress, float _end_progress)
        {
            //Get workload object from portal to perform updates once provisioned

            _task.progress(String.Format("Starting Consistency Group firedrill process"), ReportProgress.Progress(_start_progress, _end_progress, 1));
            ...
            if (_mcp_cg != null)
            {
                if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "DRS_MODE")
                {
                    ...
                    _task.progress(String.Format("Starting Consistency Group preview mode"), ReportProgress.Progress(_start_progress, _end_progress, 15));
            ...
            _task.successcomplete("Successfully configured protection group");
        }
    }
}

...
namespace MRMPService.Modules.MCP
{
    ...
    {
        public static void SetupFailoverCG(String _task_id, MRPPlatformType _platform, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, List<MRPWorkloadPairType> _workloadpairs, MRPManagementobjectSnapshotType _snapshot, float _start_progress, float _end_progress)
        {
            ...
            if (_mcp_cg != null)
            {
                if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "DRS_MODE")
                {
                    ...
                    MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Starting Consistency Group preview"), ReportProgress.Progress(_start_progress, _end_progress, 15));
            ...
            MRMPServiceBase._mrmp_api.task().progress(_task_id, "Successfully configured preview mode for consistency group", 99);

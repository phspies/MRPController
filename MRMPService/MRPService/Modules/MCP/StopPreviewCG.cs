...
namespace MRMPService.Modules.MCP
{
    ...
    {
        public static void StopPreviewCG(MRPTaskType _task, MRPPlatformType _platform, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress)
        {
            //Get workload object from portal to perform updates once provisioned
            _task.progress(String.Format("Rolling back Consistency Group firedrill"), ReportProgress.Progress(_start_progress, _end_progress, 1));
            ...
            if (_mcp_cg != null)
            {
                if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "PREVIEWING_SNAPSHOT")
                {
                    ...
                    _task.progress(String.Format("Starting Consistency Group rollback process"), ReportProgress.Progress(_start_progress, _end_progress, 15));
                    ...
                    if (_preview_status.responseCode == "IN_PROGRESS")
                    {
                        ...
                        _task.progress(String.Format("Waiting for rollback task to complete"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                        while (true)
                        {
                            _mcp_cg = (CaaS.ConsistencyGroups.GetConsistencyGroups(_filter).Result).First();
                            if (_mcp_cg.operationStatus == "DRS_MODE")
                            {
                                _task.progress(String.Format("Rollback task completed"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                                break;
                            }
                            else
                            {
            ...
            _task.progress("Successfully rolled back protection group", 99);

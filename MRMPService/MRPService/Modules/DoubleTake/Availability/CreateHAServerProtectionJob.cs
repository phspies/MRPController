    ...
    partial class Availability
    {
        public static void CreateHAServerProtectionJob(string _task_id, MRPWorkloadType _source_workload, MRPWorkloadType _target_workload, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress, String _job_type)
        {
            using (Doubletake _dt = new Doubletake(_source_workload, _target_workload))
            {
                ...
                {
                    MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("{0} - Deleting existing DR Server job between {1} and {2}", _count, _delete_job.SourceServer.Host, _delete_job.TargetServer.Host), ReportProgress.Progress(_start_progress, _end_progress, _count + 15));
                ...
                workloadId = (_dt.workload().CreateWorkload(_job_type.ToString())).Id;
                ...
                MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Successfully created disaster recover protection job between {0} to {1}", _source_workload.hostname, _target_workload.hostname), 95);

    ...
    partial class ModuleCommon
    {
        public static void DeployWindowsDoubleTake(string _task_id, MRPWorkloadType _source_workload, MRPWorkloadType _target_workload, float _start_progress, float _end_progress)
        {
            ...
            while (true)
            {
                ...
                {
                    throw new Exception(String.Format("Cannot contact workload {0} : {1}", _working_workload.hostname, _contactable_ip));
                }
                ...
                using (new Impersonator(_working_workload.credential.username, (String.IsNullOrWhiteSpace(_working_workload.credential.domain) ? "." : _working_workload.credential.domain), _working_workload.credential.encrypted_password))
                {
                    ...
                    MRMPServiceBase._mrmp_api.task().progress(_task_id, "Confirming remote CPU architecture", ReportProgress.Progress(_start_progress, _end_progress, _counter + 6));
                    ...
                    try
                    {
                        ...
                        MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("{0} is of type {1} architecture", _working_workload.hostname, systemArchitecture), ReportProgress.Progress(_start_progress, _end_progress, _counter + 7));
                    }
                    ...
                    try
                    {
                        ...
                        if (_dt_key != null)
                        {
                            ...
                            {
                                _install_dm = true;
                                MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Datamover not found on {0}", _working_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, _counter + 11));
                            }
                            else
                            {
                                MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Datamover Found on {0} : {1} [{2}] ", _working_workload.hostname, _dt_installed_version, _dt_istalled_path), ReportProgress.Progress(_start_progress, _end_progress, _counter + 10));
                                if (Versions.Compare(localFileVersion.ProductVersion, _dt_installed_version) < 0)
                                {
                                    _install_dm = true;
                                    MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Uprading datamover from {0} to {1} on {2}", _dt_installed_version, localFileVersion.ProductVersion, _working_workload.hostname, _dt_installed_version, _dt_istalled_path), ReportProgress.Progress(_start_progress, _end_progress, _counter + 11));
                                }
                                else
                                {
                                    MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Testing datamover connectivity on {0} using {1}", _working_workload.hostname, _contactable_ip), ReportProgress.Progress(_start_progress, _end_progress, _counter + 14));
                                    ...
                                    using (Doubletake _dt = new Doubletake(null, _working_workload))
                                    {
                                        try
                                        {
                                            ...
                                            MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Datamover installed and running on {0} with version {1}", _working_workload.hostname, _dt.management().GetProductVersion(_installed_dt_version)), ReportProgress.Progress(_start_progress, _end_progress, _counter + 15));
                                        }
                                        catch (Exception ex)
                                        {
                                            _install_dm = true;
                                            MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Datamover installed on {0} but cannot be contacted: {1} : {2}", _working_workload.hostname, ex.Message, _working_workload, _contactable_ip), ReportProgress.Progress(_start_progress, _end_progress, _counter + 15));
                                        }
                                    }
                                }
                            }
                    ...
                    if (_install_dm)
                    {
                        ...
                        MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Copy installation file to {0} on {1} ({2})", remoteTempLocation, _working_workload.hostname, systemArchitecture), ReportProgress.Progress(_start_progress, _end_progress, _counter + 20));
                        ...
                        using (Doubletake _dt = new Doubletake(null, _working_workload))
                        {
                            while (true)
                            {
                                try
                                {
                                    ...
                                    MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Datamover {0} found on {1}", _dt.management().GetProductVersion(_dt_version), _working_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, _counter + 45));
                                    break;
                                }
                                catch (Exception)
                                {
                                ...
                                MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Verify datamover connectivity on {0} [{1}]", _working_workload.hostname, restries), ReportProgress.Progress(_start_progress, _end_progress, _counter + 41 + restries));
                            }

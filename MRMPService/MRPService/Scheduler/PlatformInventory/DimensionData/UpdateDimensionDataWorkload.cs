...
namespace MRMPService.Scheduler.PlatformInventory.DimensionData
{
    partial class PlatformInventoryWorkloadDo
    {
        public static async Task UpdateMCPWorkload(String _workload_moid, MRPPlatformType _platform)
        {
            ...
            MRPOrganizationType _org_tags = await MRMPServiceBase._mrmp_api.organization().get();
            try
            {
            ...
            else
            {
                ...
                await MRMPServiceBase._mrmp_api.workload().createworkload(_update_workload);
            }
        }
    }
}

                ...
                await System.Threading.Tasks.Task.Run(() =>
                {
                    ...
                    else if ((common._version == 10))
                    {
                        if (_bytes.Count() > 16)
                        {
                            ...
                            if (_flowset != null)
                            {
                                foreach (Modules.Netflow.v10.Template _template in _flowset.Template.Where(x => x.Field.Any(y => y.Value.Count != 0)))
                                {
                                    NetworkFlow _netflow = new NetworkFlow();

                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV4_SRC_ADDR))
                                    {
                                        _netflow.source_address = new IPAddress(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV4_SRC_ADDR).Value.ToArray()).ToString();
                                        _netflow.target_address = new IPAddress(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV4_DST_ADDR).Value.ToArray()).ToString();
                                    }
                                    else if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV6_SRC_ADDR))
                                    {
                                        _netflow.source_address = new IPAddress(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV6_SRC_ADDR).Value.ToArray()).ToString();
                                        _netflow.target_address = new IPAddress(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IPV6_DST_ADDR).Value.ToArray()).ToString();
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.L4_SRC_PORT))
                                    {
                                        _netflow.source_port = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.L4_SRC_PORT).Value.ToArray().Reverse().ToArray(), 0);
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.L4_DST_PORT))
                                    {
                                        _netflow.target_port = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.L4_DST_PORT).Value.ToArray().Reverse().ToArray(), 0);
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.PROTOCOL))
                                    {
                                        _netflow.protocol = _template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.PROTOCOL).Value[0];
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.FIRST_SWITCHED))
                                    {
                                        _netflow.start_timestamp = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.FIRST_SWITCHED).Value.ToArray().Reverse().ToArray(), 0);
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.LAST_SWITCHED))
                                    {
                                        _netflow.stop_timestamp = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.LAST_SWITCHED).Value.ToArray().Reverse().ToArray(), 0);
                                    }
                                    _netflow.timestamp = DateTime.UtcNow;
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IN_PKTS))
                                    {
                                        _netflow.packets = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IN_PKTS).Value.ToArray().Reverse().ToArray(), 0);
                                    }
                                    if (_template.Field.Exists(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IN_BYTES))
                                    {
                                        _netflow.kbyte = BitConverter.ToUInt16(_template.Field.FirstOrDefault(x => x.GetTypes() == (UInt16)Modules.Netflow.v10.FieldType.IN_BYTES).Value.ToArray().Reverse().ToArray(), 0);
                                    }

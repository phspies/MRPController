        ...
        public async void Start()
        {
            try
            {
                while (true)
                {
                    ...
                    Logger.log(String.Format("Netstat: Staring netstat collection process with {0} threads", MRMPServiceBase.os_netstat_concurrency), Logger.Severity.Info);

                    Parallel.ForEach(_all_workloads, new ParallelOptions() { MaxDegreeOfParallelism = MRMPServiceBase.os_netstat_concurrency }, async workload =>
                    {
                        try
                        {

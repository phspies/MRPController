...
namespace MRMPService.Tasks.MCP
{
    partial class MCP_Platform
    {
        public static async Task StopPreviewCG(String _task_id, MRPPlatformType _platform, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress)
        {
            ...
            await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Rolling back Consistency Group firedrill"), ReportProgress.Progress(_start_progress, _end_progress, 1));
            ...
            if (_mcp_cg != null)
            {
                if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "PREVIEWING_SNAPSHOT" && _mcp_cg.drsInfrastructure.status == "ACTIVE")
                {
                    ...
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Starting Consistency Group rollback process"), ReportProgress.Progress(_start_progress, _end_progress, 15));
                    ...
                    if (_preview_status.responseCode == "IN_PROGRESS")
                    {
                        ...
                        await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Waiting for rollback task to complete"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                        while (true)
                        {
                            _mcp_cg = (await CaaS.ConsistencyGroups.GetConsistencyGroups(_filter)).First();
                            if (_mcp_cg.operationStatus == "DRS_MODE")
                            {
                                await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Rollback task completed"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                                break;
                            }
                            else
                            {
            ...
            await MRMPServiceBase._mrmp_api.task().progress(_task_id, "Successfully rolled back protection group", 99);

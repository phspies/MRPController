            ...
            using (MRMPAPI.MRMP_ApiClient _mrp_api = new MRMPAPI.MRMP_ApiClient())
            {
                ...
                if (_mcp_cg != null)
                {
                    if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "DRS_MODE" && _mcp_cg.drsInfrastructure.status == "ACTIVE")
                    {
                        ...
                        _mrp_api.task().progress(_task_id, String.Format("Starting Consistency Group preview"), ReportProgress.Progress(_start_progress, _end_progress, 15));
                ...
                _mrp_api.task().progress(_task_id, "Successfully configured preview mode for consistency group",99);

...
namespace MRMPService.Tasks.MCP
{
    partial class MCP_Platform
    {
        public static async Task CreateCG(String _task_id, MRPPlatformType _platform, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, List<MRPWorkloadPairType> _workloadpairs, float _start_progress, float _end_progress)
        {
            ...
            await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Starting Consistency Group configuration process"), ReportProgress.Progress(_start_progress, _end_progress, 1));
            ...
            else
            {
                ...
                if (_current_cgs != null)
                {
                    ...
                    {
                        await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Consistency group changed, removing and recreating consistency group"), ReportProgress.Progress(_start_progress, _end_progress, 2));
                        ...
                        {
                            await MRMPServiceBase._mrmp_api.task().progress(_task_id, _delete_result.message, 3);
                            ...
                            await MRMPServiceBase._mrmp_api.task().progress(_task_id, "Consistency group successfully removed", ReportProgress.Progress(_start_progress, _end_progress, 4));
                            _deploy_cg = true;
                    ...
                    {
                        await MRMPServiceBase._mrmp_api.task().progress(_task_id, "Consistency group remains unchanged", 99);
                    }
                }
                else
                {
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, "Consistency group does not exist in platform", ReportProgress.Progress(_start_progress, _end_progress, 4));
                    _deploy_cg = true;
                }
            }
            if (_deploy_cg)
            {
                try
                {
                    ...
                    foreach (var _workloadpair in _workloadpairs)
                    {
                        ...
                        MRPWorkloadType _updated_workload = await MRMPServiceBase._mrmp_api.workload().get_by_id(_workloadpair.source_workload.id);
                        ...
                        if (!_cass_server.started)
                        {
                            ...
                            {
                                await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Workload {0} stared", _updated_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, _progress + 6));
                            }
                            else
                            {
                        ...
                        {
                            //get new copy of workload from portal
                            _updated_workload = await MRMPServiceBase._mrmp_api.workload().get_by_id(_workload.id);
                            ...
                            while (true)
                            {
                                ...
                                if ((!(_cass_server.drsEligible is drsEligible)) && _cass_server.consistencyGroup == null)
                                {
                                    ...
                                    {
                                        await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Workload {0} not eligible for DRS. Setting meta information for workload", _updated_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, _progress + 5));
                                        try
                                        {
                                            ...
                                            await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("{0}", _meta_result.message), _progress + 10);
                        ...
                        var _source_workload = await MRMPServiceBase._mrmp_api.workload().get_by_id(_workloadpair.source_workload.id);
                        var _target_workload = await MRMPServiceBase._mrmp_api.workload().get_by_id(_workloadpair.target_workload.id);
                    ...
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Creating consitency group with a total of {0} server pairs", _mcp_server_pairs.Count), ReportProgress.Progress(_start_progress, _end_progress, 50));
                    ...
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("{0} : {1}", _cg_create_response.message, _cg_moid), 51);
                    ...
                    while (_cg_status.state != "NORMAL")
                    {
                        ...
                        {
                            await MRMPServiceBase._mrmp_api.task().progress(_task_id, _cg_status.progress.step.name, ReportProgress.Progress(_start_progress, _end_progress, 55 + _cg_status.progress.step.number));
                        }
                        Thread.Sleep(TimeSpan.FromSeconds(5));
                        _cg_status = (await CaaS.ConsistencyGroups.GetConsistencyGroups(new ConsistencyGroupListOptions() { Id = new Guid(_cg_moid) })).First();
                    }

                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Registering contency group {0} with portal", _cg_moid), ReportProgress.Progress(_start_progress, _end_progress, 91));
                    ...
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, "Successfully configured protection group", 99);
                    await MCPCGPoller.PollerDo(new MRPManagementobjectType()
                    {
                        id = _managementobject.id,
                        moid = _cg_moid,
                        moname = _protectiongroup.group,

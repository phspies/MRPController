    ...
    partial class MCP_Platform
    {
        public static async Task ApplyMetaInformation(String _task_id, List<MRPWorkloadPairType> _workload_pairs, MRPPlatformType _source_platform, MRPPlatformType _target_platform, MRPWorkloadType _source_workload, MRPWorkloadType _target_workload, float _start_progress, float _end_progress)
        {
            ...
            using (MRMPAPI.MRMP_ApiClient _mrp_api = new MRMPAPI.MRMP_ApiClient())
            {
                ...
                {
                    _mrp_api.task().progress(_task_id, String.Format("Applying workload tags to {0} from {1}", _target_workload.hostname, _source_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 5));
                    ...
                    if (_source_tags != null)
                    {
                        ...
                        if (_add_platform_tags.Count > 0)
                        {
                            ...
                            {
                                _mrp_api.task().progress(_task_id, String.Format("Successfully applied {0} platform tags to {1}", _add_platform_tags.Count(), _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 10));
                            }
                            else
                            {
                                throw new Exception(String.Format("Error applying user tags for {0} : {1}", _target_workload.hostname, _tag_platform_task.message));
                    ...
                    List<MRPOrganizationTagType> _org_tags = _mrp_api.organization().get().organizationtags;
                    ...
                    foreach (var _org_tag in _org_tags)
                    {
                        ...
                        if (_create_tag)
                        {
                            ...
                            if (_create_org_tag.responseCode == "OK")
                            {
                                ...
                                _mrp_api.organization().update(new MRPOrganizationCRUDType()
                                {
                    ...
                    _org_tags = _mrp_api.organization().get().organizationtags;
                    ...
                    if (_add_user_tags.Count > 0)
                    {
                        ...
                        {
                            _mrp_api.task().progress(_task_id, String.Format("Successfully applied {0} user tags to {1}", _add_user_tags.Count(), _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 15));
                        }
                        else
                        {
                            throw new Exception(String.Format("Error applying user tags for {0} : {1}", _target_workload.hostname, _tag_task.message));
                ...
                _mrp_api.task().progress(_task_id, "Successfully configured workload meta data", ReportProgress.Progress(_start_progress, _end_progress, 29));


                _mrp_api.task().progress(_task_id, String.Format("Applying workload affinity rules to {0}", _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 30));
                ...
                if ((bool)_target_workload.sync_affinity_rules)
                {
                    ...
                    foreach (var _target_rule in _target_affinity_rules.Where(x => x.affinitytype == "user"))
                    {
                        ...
                        if (_create_user_rule)
                        {
                            ...
                            {
                                _mrp_api.task().progress(_task_id, String.Format("Error applying affinity rule between {0} and {1} : {2}", _target_rule.workload1_id, _target_rule.workload2_id, ex.GetBaseException().Message), ReportProgress.Progress(_start_progress, _end_progress, _target_affinity_rules.IndexOf(_target_rule) + 31));
                            }
                        }
                    }

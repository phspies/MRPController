...
namespace MRMPService.Tasks.MCP
{
    partial class MCP_Platform
    {
        public static async Task PreviewCG(String _task_id, MRPPlatformType _platform, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, List<MRPWorkloadPairType> _workloadpairs, MRPManagementobjectSnapshotType _snapshot, float _start_progress, float _end_progress)
        {
            ...
            await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Starting Consistency Group firedrill process"), ReportProgress.Progress(_start_progress, _end_progress, 1));
            ...
            if (_mcp_cg != null)
            {
                if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "DRS_MODE" && _mcp_cg.drsInfrastructure.status == "ACTIVE")
                {
                    ...
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Starting Consistency Group preview mode"), ReportProgress.Progress(_start_progress, _end_progress, 15));

...
namespace MRMPService.Tasks.MCP
{
    ...
    {
        public static async Task PreviewCG(String _task_id, MRPPlatformType _platform, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, List<MRPWorkloadPairType> _workloadpairs, MRPManagementobjectSnapshotType _snapshot, float _start_progress, float _end_progress)
        {
            ...
            {
                _mrp_api.task().progress(_task_id, String.Format("Starting Consistency Group firedrill process"), ReportProgress.Progress(_start_progress, _end_progress, 1));
                ...
                if (_mcp_cg != null)
                {
                    if (_mcp_cg.state == "NORMAL" && _mcp_cg.operationStatus == "DRS_MODE" && _mcp_cg.drsInfrastructure.status == "ACTIVE")
                    {
                        ...
                        _mrp_api.task().progress(_task_id, String.Format("Starting Consistency Group preview mode"), ReportProgress.Progress(_start_progress, _end_progress, 15));
                        ...
                        if (_preview_status.responseCode == "IN_PROGRESS")
                        {
                            ...
                            _mrp_api.task().progress(_task_id, String.Format("Waiting for preview mode task to complete"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                            while (true)
                            {
                                _mcp_cg = (await CaaS.ConsistencyGroups.GetConsistencyGroups(_filter)).First();
                                if (_mcp_cg.operationStatus == "PREVIEWING_SNAPSHOT")
                                {
                                    _mrp_api.task().progress(_task_id, String.Format("Preview mode task completed"), ReportProgress.Progress(_start_progress, _end_progress, 16));
                                    break;
                                }
                                else
                                {

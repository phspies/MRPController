...
namespace MRMPService.Tasks.DoubleTake
{
    partial class ModuleCommon
    {
        public static async Task PauseJob(string _task_id, MRPWorkloadType _target_workload, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress)
        {
            ...
            await MRMPServiceBase._mrmp_api.task().progress(_task_id, string.Format("Pausing job {0} on {1}", _managementobject.moname, _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 10));
            ...
            await MRMPServiceBase._mrmp_api.task().successcomplete(_task_id, String.Format("Job paused successfully {0} on {1} : {2}", _managementobject.moname, _target_workload.hostname, _dt_job.Status.HighLevelState));
        }
    }
}

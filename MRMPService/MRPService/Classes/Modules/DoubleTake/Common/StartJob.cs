    ...
    partial class DT_Common
    {
        public static void StartJob(string _task_id, MRPWorkloadType _target_workload, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress)
        {
            using (MRMPAPI.MRMP_ApiClient _mrp_portal = new MRMPAPI.MRMP_ApiClient())
            {
                ...
                _mrp_portal.task().progress(_task_id, String.Format("Starting job {0} on {1}", _managementobject.moname, _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, 10));
                ...
                using (Doubletake _dt = new Doubletake(null, _target_workload))
                {
                    ...
                    {
                        _dt.job().StartJob((Guid)_managementobject.moid).Wait();
                    }
                    else
                    {
                ...
                _mrp_portal.task().successcomplete(_task_id, String.Format("Job Started successfully {0} on {1} : {2}", _managementobject.moname, _target_workload.hostname, _dt_job.Status.HighLevelState));

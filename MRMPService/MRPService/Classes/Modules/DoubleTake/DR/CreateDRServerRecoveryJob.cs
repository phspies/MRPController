...
namespace MRMPService.Tasks.DoubleTake
{
    ...
    {
        public static async Task CreateDRServerRecoveryJob(string _task_id, MRPWorkloadType _source_workload, MRPWorkloadType _target_workload, MRPWorkloadType _original_workload, MRPProtectiongroupType _protectiongroup, MRPManagementobjectType _managementobject, float _start_progress, float _end_progress)
        {
            ...
            using (Doubletake _dt = new Doubletake(_source_workload, _target_workload))
            {
                ...
                {
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("{0} - Deleting existing DR Server job between {1} and {2}", _count, _source_workload.hostname, _target_workload.hostname), ReportProgress.Progress(_start_progress, _end_progress, _count + 15));
                    await _dt.job().DeleteJob(_delete_job.Id);
                    _count += 1;
                }
                ...
                {
                    await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Recovering data from snapshot taken at {0}", _managementobject.managementobjectsnapshot.timestamp), ReportProgress.Progress(_start_progress, _end_progress, 19));

                    wkld = await _dt.workload().CreateWorkloadDRRecovery(Guid.Parse(_managementobject.managementobjectsnapshot.imagemoid), Guid.Parse(_managementobject.managementobjectsnapshot.snapshotmoid));
                }
                else
                {
                    IEnumerable<ImageInfoModel> images = await _dt.image().GetAllImagesFromSource();
                    ...
                    wkld = await _dt.workload().CreateWorkloadDRRecovery(image.Id, Guid.Empty);
                }
                ...
                jobInfo = await SetOptions.set_job_options(_task_id, _source_workload, _target_workload, _protectiongroup, jobInfo, 40, 59, _managementobject);
                ...
                await MRMPServiceBase._mrmp_api.task().progress(_task_id, String.Format("Successfully created disaster recovery job between {0} to {1}", _source_workload.hostname, _target_workload.hostname), 95);

            ...
            using (MRMP_ApiClient _mrp_api = new MRMPAPI.MRMP_ApiClient())
            {
                try
                {
                    using (Doubletake _dt = new Doubletake(_source_workload, _target_workload))
                    {
                        ...
                        _mrp_api.task().progress(_task_id, "Looking for migration jobs on target workload", 11);
                        int _count = 1;
                        ...
                        {
                            _mrp_api.task().progress(_task_id, String.Format("{0} - Deleting existing migration job between {1} and {2}", _count, _source_workload.hostname, _target_workload.hostname), _count + 15);
                        ...
                        else
                        {
                            ...
                            {
                                _mrp_api.task().progress(_task_id, String.Format("Error creating job: {0} : {1}", _failed_item.TitleKey, _failed_item.MessageKey), _fix_count + 55);
                        ...
                        _mrp_api.task().progress(_task_id, String.Format("Job created. Starting job id {0}", jobId), 57);
                        _dt.job().StartJob(jobId).Wait();

                        _mrp_api.task().progress(_task_id, String.Format("Registering job {0} with portal", jobId), 60);
                        ...
                        _mrp_api.task().progress(_task_id, String.Format("Sync process started at {0}", jobinfo.Statistics.CoreConnectionDetails.StartTime), 70);

                        _mrp_api.task().progress(_task_id, String.Format("Successfully created move synchronization job between {0} to {1}", _source_workload.hostname, _target_workload.hostname), 95);
